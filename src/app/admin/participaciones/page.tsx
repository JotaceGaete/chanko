...
import { ArrowLeft, ThumbsUp, ThumbsDown, HelpCircle, Users } from "lucide-react"
import { participaciones, estadisticas } from "@/lib/mock-data"
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-8">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-sm text-amber-800">
          <strong>Privacidad:</strong> El RUT y correo electrónico de los participantes no se muestran en esta interfaz. Solo el equipo técnico con acceso a la base de datos puede consultarlos.
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="text-left px-4 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wide">Posición</th>
                  ...
                  <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 bg-verde-100 rounded-full flex items-center justify-center text-verde-700 text-xs font-bold">
                          {p.nombre.charAt(p.nombre.length > 11 ? 10 : 0)}
                        </div>
                    ...
                    <td className="px-4 py-3.5">
                      <span className={`badge text-xs flex items-center gap-1 w-fit ${cfg.badge}`}>
                    ...
                    <td className="px-4 py-3.5 hidden md:table-cell">
                      {p.comentario ? (
                        <div>
                          <span className={`badge text-xs mb-1 inline-block ${
                            p.estadoComentario === "aprobado" ? "bg-verde-100 text-verde-700"
                            : p.estadoComentario === "rechazado" ? "bg-red-100 text-red-700"
                            : "bg-amber-100 text-amber-700"
                          }`}>
                            {p.estadoComentario}
                          </span>
                          <p className="text-gray-400 text-xs line-clamp-1">&ldquo;{p.comentario}&rdquo;</p>
                        </div>
                      ) : (
                        <span className="text-gray-300 text-xs">Sin comentario</span>
                      )}
                    </td>
                    <td className="px-4 py-3.5 text-sm text-gray-400 hidden lg:table-cell">
                      {formatDate(p.fecha)}
                    </td>
                  </tr>
                )
        ...
        <div className="px-5 py-4 border-t border-gray-100 bg-gray-50 text-sm text-gray-400">
          Mostrando 50 de {participaciones.length} participaciones
        </div>
      </div>
    </div>
  )
